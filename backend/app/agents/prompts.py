...
"""
    return f"""Generate 6 clearly distinct arguments. Every argument must support "the claim you must argue" for your side, with ids R1 through R6.
...
- question、weakness_reason、opposing_reason 都必须具体指向被选中的逻辑链条。"""
        ...
        system = f"""/no_think
You are challenging agent {agent_index} in round two, representing the side opposite to the target side. Output valid JSON only, with no Markdown, explanation, or hidden reasoning."""
        user = f"""Motion: {topic}
Your represented side: {ROLE_LABELS[language][challenge_side]}
You are challenging the 6 reasons generated by: {ROLE_LABELS[language][target_side]}
...
Return this JSON object:
{{"target_id":"R1","question":"the challenge question for this logic","weakness_reason":"why it is the weakest","opposing_reason":"the reason against it from your represented side's stance"}}
...
- opposing_reason must be based on your represented side's stance, not the target side's stance."""
    return [{"role": "system", "content": system}, {"role": "user", "content": user}]
...
Preserve the core direction of the original reason. After optimization, the result must still argue "the claim you must argue" for your side and must not drift toward the other side.
logic_chain: {EN_LOGIC_CHAIN_OPTIMIZE_DESC}; it must absorb all challenges and rebuttal content.
